...
import { createRegistro, getCurvas } from '../../services/api';
...
const FormularioMedicion = ({ zona, onClose, onSuccess }) => {
  ...
  const calcularInterpolacion = (valorX, puntos) => {
    ...
    if (valorX < sorted[0][0] || valorX > sorted[sorted.length - 1][0]) {
      throw new Error(`El valor ${valorX} está fuera del rango (${sorted[0][0]} a ${sorted[sorted.length - 1][0]}).`);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (mediciones.length === 0) {
      setError("Debe agregar al menos una medición para calcular el promedio.");
      return;
    }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div style={{ padding: '1rem', border: '1px solid var(--border-color)', borderRadius: '0.5rem', backgroundColor: 'rgba(0,0,0,0.1)' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', alignItems: 'flex-end' }}>
              ...
              <div style={{ flex: 1 }}>
                <label style={{ display: 'block', marginBottom: '0.25rem', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Curva</label>
